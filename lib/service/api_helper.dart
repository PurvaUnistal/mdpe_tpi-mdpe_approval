...
import 'package:mdpe_approve_app/Utils/common_widgets/common_session_dialog_box.dart';
...
class ApiHelper {
  static Future<dynamic> getData({var urlEndPoint, required BuildContext context}) async {
    try {
      ...
      if (e is SocketException) {
        ...
        UtilsToast.errorSnackBar(msg: "No Internet", context: context);
      } else if (e is TimeoutException) {
        log("TimeoutException : ${e.toString()}");
        UtilsToast.errorSnackBar(msg: "Timeout, Please try again", context: context);
      } else {
  ...
  static Future<dynamic> postDataWithFile({
    ...
    String token =  await AppConfig.instanceInit()?.loginData.token! ?? "";
    try {
      if(await ConnectivityHelper.allConnectivityCheck(context: context) == false){
        return null;
      }
      ...
      if (response.statusCode == 200) {
        ...
        UtilsToast.errorSnackBar(msg: result['data'].toString(), context:context);
